    ...
    public class SelfContainedDBOps
    {
        ...
        public void NoInitNullCollectionsAddAndSave()
        {
            using (CEF.NewServiceScope())
            {
                ...
                p1.Age = 22;
                p1.Gender = "M";
                p1.Phones = CEF.CreateList(p1, nameof(Person.Phones), DataRowState.Added, new Phone() { Number = "111-2222", PhoneTypeID = PhoneType.Home, Owner = p1 }, new Phone() { Number = "222-3333", PhoneTypeID = PhoneType.Mobile, Owner = p1 });
                var p2 = CEF.NewObject<PersonWrapped>();
                p2.Name = "Mary";
                p2.Age = 2;
                p2.Gender = "F";
                p1.Kids = CEF.CreateList<Person>(p1, nameof(Person.Kids), DataRowState.Added, p2);
                Assert.AreEqual(4, CEF.DBSave().Count());
                var ph1 = (from a in p1.Phones where a.PhoneTypeID == PhoneType.Home select a).First();
                ph1.Owner = null;
                p1.Phones.Remove(ph1);
                p2.Age = 1;
                Assert.AreEqual(2, CEF.DBSave().Count());
                p2.Phones = CEF.CreateList(p2, nameof(Person.Phones), DataRowState.Modified, ph1);
        ...
        public void PopulateFromInitialPocoVariousRetrievalsSaves()
        {
            using (CEF.NewServiceScope())
            {
                ...
                Assert.AreEqual(2, (from a in es2 where a.AsInfraWrapped().GetRowState() == DataRowState.Unchanged select a).Count());
