...
namespace CodexMicroORM.Providers
{
    ...
    {
        private const string DISCOVER_RETRY_MESSAGE_REGEX = @"timeout\sexpired|\spool\s|successfully\sestablished|was\snot\sfound|was\snot\saccessible|not\scurrently\savailable|is\snot\susable|please\sretry|too\sbusy";
        ...
        public static int? DiscoverRetryCount
        {
            get;
            set;
        }

        public static int DiscoverRetryDelayMs
        {
            get;
            set;
        } = 1000;
        ...
        {
            int trycnt = DiscoverRetryCount.GetValueOrDefault(0) + 1;
            int trydelay = 0;
            ...
            while (1 == 1)
            {
                try
                {
                    ...
                    var (schema, name) = _cmd.CommandText.SplitIntoSchemaAndName();
                ...
                catch (Exception ex)
                {
                    ...
                    if (trycnt == 0 || !Regex.IsMatch(ex.Message, DISCOVER_RETRY_MESSAGE_REGEX, RegexOptions.IgnoreCase))
                    {
#if DEBUG
